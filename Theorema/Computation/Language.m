...
(* Instead of nesting SequenceOf expressions and then concatenating \
the sequences, we construct a multi-iterator from the given ranges *)

SequenceOf$TM[ RNG$[ r__], cond_, form_] :=
 	Module[ {s},
  		s /; buiActive[
     "SequenceOf"] && (s = 
       sequenceOfIteration[ Map[ rangeToIterator, {r}], cond, 
        form]) =!= $Failed
  	]
